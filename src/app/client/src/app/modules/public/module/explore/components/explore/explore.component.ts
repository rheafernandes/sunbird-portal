...
import { PageApiService, PlayerService, ISort, OrgDetailsService, SearchService,
  CoursesService, ICourses, SearchParam, FrameworkService } from '@sunbird/core';
import { PublicPlayerService } from './../../../../services';
import { Component, OnInit, OnDestroy, NgZone, ChangeDetectorRef, EventEmitter, Output, ViewChild  } from '@angular/core';
import {
...
constructor(searchService: SearchService, route: Router, private playerService: PlayerService,
  ...
  config: ConfigService, coursesService: CoursesService, public utilService: UtilService, private orgDetailsService: OrgDetailsService,
    ...
    this.orgDetailsService.getOrgDetails(this.slug).pipe(
      ...
      .subscribe(
        (apiResponse: any) => {
          ...
          // this.frameworkService.frameworkData$.subscribe(console.log);
        },
        err => {
          this.route.navigate(['']);
        }
...
populateCourseSearch() {
  ...
  const requestParams = {
    filters: filters , // _.pickBy(this.filters, value => value.length > 0)
...
navigateToPage(page: number): undefined | void {
  ...
  this.route.navigate(['/explore/catalog', this.pageNumber], {
